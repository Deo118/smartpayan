/*  
  SmartPayan v4 — ESP32 Firmware  
  Automatic + Manual IoT Clothesline System  
  Full Firebase RTDB Sync • Supabase Notifications • Weather-Based Control • Heartbeat System

  ---------------------------------------------------------------------------
  OVERVIEW
  ---------------------------------------------------------------------------
  This firmware controls an IoT clothesline system (SmartPayan) using an ESP32.  
  It synchronizes with Firebase Realtime Database, sends push notifications via  
  Supabase Edge Functions, monitors weather sensors, and allows both automatic  
  and manual user control from a mobile app.

  ---------------------------------------------------------------------------
  FEATURES
  ---------------------------------------------------------------------------

  1. **Automatic Clothesline Control**
     - Uses DHT22 (temperature & humidity), BH1750 (light), and rain sensors.
     - Automatically retracts on:
         • Rain detection  
         • Low light levels  
         • High humidity  
     - Automatically extends during good weather.

  2. **Manual Control from App**
     - User slider in the app forces manual extend or retract.
     - Manual actions send special notifications:
         • "User extended the clothesline manually via the app."
         • "User retracted the clothesline manually via the app."

  3. **Realtime Sync with Firebase RTDB**
     - Reads user commands (autoMode, clotheslinePosition).
     - Updates current device state and sensor readings:
         • temperature  
         • humidity  
         • light level  
         • rain status  
         • clothesline state (“extended”, “retracted”, “moving”)

  4. **Supabase Cloud Integrations**
     - Sends push notifications for:
         • Automatic retraction (with cause: rain, low light, humidity, or combinations)
         • Automatic extension (good weather)
         • Manual control actions
         • Device online status  
     - Uses ANON KEY for secure client-side access.
  ...
  5. **Heartbeat System**
     ...
     - Allows server to track device online/offline status.

  6. **Stability & Safety**
     - WiFi status check before all cloud operations.
     - Brownout-safe (no blocking network operations during low voltage).
     - Prevents repeated RTDB writes & repeated notifications.
     - Maintains app and ESP32 state consistency.
  ...
  • Motor Driver (IN1, IN2, ENA)  
  • Real-time cloud integrations (Firebase + Supabase)  
